...
// -l (padr~ao de acesso e localidade)
...
// Saida: optescolhida, optx, opty, regmem, lognome
{
		 ...
		 while ((c = getopt(argc, argv, "smtp:o:1:2:lh")) != EOF)
			...
			avisoAssert(opescolhida==-1,"Mais de uma operacao escolhida");
				 ...
				 case 'o':
						strcpy(matrizResult,optarg);
				 case '1': 
						strcpy(matrizOne,optarg);
				 case '2':
						strcpy(matrizTwo,optarg);
			break;
				 case 'x': 
						optx = atoi(optarg);
			break;
...
// Saida: depende da operacao escolhida
{
	...
	switch (opescolhida){
	 ...
	 defineFaseMemLog(0);
				 criaMatrizArquivo(&a,matrizOne,0);
				 criaMatrizArquivo(&b,matrizTwo,1);
				 criaMatrizArquivo(&c,matrizTwo,2);
	 ...
	 defineFaseMemLog(0);
				 criaMatrizArquivo(&a,matrizOne,0);
				 criaMatrizArquivo(&b,matrizOne,1);
				 criaMatrizArquivo(&c,matrizOne,2);
	 defineFaseMemLog(1);
				 acessaMatriz(&a);
				 acessaMatriz(&b);
				 acessaMatriz(&c);
