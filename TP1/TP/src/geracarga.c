...
int main(int argc, char ** argv){
  ...
  int dinheiroBase = 50 * prm.rodadas * prm.jogadores/2;
  fprintf(out,"%d %d\n",prm.rodadas, dinheiroBase);

  int maxJogadores = prm.jogadores + prm.varjogadores;
  // gera carga de trabalho
  for (i=0; i<prm.rodadas; i++){
    // sorteia numero de jogadores
    if(i==0) numjogadores = maxJogadores;
    else numjogadores = (int) RandN2(prm.jogadores,prm.varjogadores);
